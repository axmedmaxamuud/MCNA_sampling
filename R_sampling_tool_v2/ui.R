		...
		fluidRow(
			...
			column(4,
				h2("Loading files"),
					...
					p(strong("Population:"),"The variable in the data defining the population nummber by PSU."),
	...
	tabPanel("Sampling frame",
		fluidRow(
		 h4("Sampling parameters",style="color:#707070;background-color:#f8f8f8"),
				...
				column(1,
					checkboxInput("stratified", "Stratified", TRUE)
				),
				column(2,						
					numericInput('conf_level', 'Confidence level',0.9,min=0.80,	max=0.99, step =0.005)
				),
				column(2,						
					numericInput('e_marg', 'Error Margin',0.1,min=0.01,	max=0.25, step =0.005)
				),
				column(2,						
					numericInput('pror', 'Proportion',0.5,min=0.01,	max=1, step =0.05)
				),
				column(2,						
					numericInput('buf', 'Buffer',0.03,min=0.01,	max=0.25, step =0.01)
				)
		),
		fluidRow(
			column(6,	
				conditionalPanel(condition = "input.samp_type == 'Cluster sampling'",
					...
					column(6,						
						numericInput('cls', 'Min cluster size',6,min=2, step =1)
					),
					column(6,						
			    ...
			    column(6,
					...
					checkboxInput("testdata", "Use test data", TRUE),
					conditionalPanel(condition = "input.samp_type == 'Cluster sampling'",
						selectInput('col_psu', 'Input cluster', choices = NULL)
				)),
				column(6,					
					conditionalPanel(condition = "input.stratified",
